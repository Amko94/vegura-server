...
function Player.transferMoneyTo(self, target, amount)
	...
	local senderBalance = self:getBankBalance()
	if amount > senderBalance then
		return false
	end
	...
	if targetPlayer then
		local receiverBalance = targetPlayer:getBankBalance()
		local newReceiverBalance = receiverBalance + amount
		targetPlayer:setBankBalance(newReceiverBalance)


		db.query(string.format(
				"UPDATE `players` SET `balance` = %d WHERE `id` = %d",
				newReceiverBalance, targetPlayer:getGuid()
		))
	else
		...
		db.query(string.format(
				...
				amount, db.escapeString(target)
		))
	end

	local newSenderBalance = senderBalance - amount
	self:setBankBalance(newSenderBalance)
	db.query(string.format(
			"UPDATE `players` SET `balance` = %d WHERE `id` = %d",
			newSenderBalance, self:getGuid()
	))
