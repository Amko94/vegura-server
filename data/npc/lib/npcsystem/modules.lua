...
if Modules == nil then
	...
	function StdModule.say(cid, message, keywords, parameters, node)
		...
		local parseInfo = {[TAG_PLAYERNAME] = player:getName(), [TAG_TIME] = getFormattedWorldTime(), [TAG_BLESSCOST] = getBlessingsCost(player:getLevel()), [TAG_TRAVELCOST] = costMessage}
	...
	function ShopModule:addBuyableItem(names, itemid, cost, itemSubType, realName)
		...
		if names ~= nil and SHOPMODULE_MODE ~= SHOPMODULE_MODE_TRADE then
			for i = 1, #names do
				...
				local keywordsWithBuy = { "buy", names[i] }
				local nodeWithBuy = self.npcHandler.keywordHandler:addKeyword(keywordsWithBuy, ShopModule.tradeItem, parameters)
				nodeWithBuy:addChildKeywordNode(self.yesNode)
				nodeWithBuy:addChildKeywordNode(self.noNode)

				-- Ohne "buy"
				local keywordsWithoutBuy = { names[i] }
				local nodeWithoutBuy = self.npcHandler.keywordHandler:addKeyword(keywordsWithoutBuy, ShopModule.tradeItem, parameters)
				nodeWithoutBuy:addChildKeywordNode(self.yesNode)
				nodeWithoutBuy:addChildKeywordNode(self.noNode)
			end
		end
	...
	function ShopModule.requestTrade(cid, message, keywords, parameters, node)
		...
		if(itemWindow[1] == nil) then
			...
			return true
                ...
                if SHOPMODULE_MODE == SHOPMODULE_MODE_BOTH then
                        local sellableItems, buyableItems = {}, {}
                        for i, shopItem in ipairs(module.npcHandler.shopItems) do
                                if shopItem.sell >= 0 then
					sellableItems[#sellableItems + 1] = shopItem.name
                                end
                                if shopItem.buy >= 0 then
					buyableItems[#buyableItems + 1] = shopItem.name
                                end
                        end

			local npc = Npc()	
                        if table.maxn(buyableItems) > 0 then
                        	local message = "I'm selling "
                        	for i = 1, table.maxn(buyableItems) do
                        		message = message .. buyableItems[i] .. ", "
                        		if message:len() > 500 then
                        			npc:say(message:sub(0, -3) .. ".", TALKTYPE_SAY)
                        			message = "I'm selling "
                        		end
                        	end
