...
Spells::Spells() : scriptInterface("Spell Interface") {
...
bool Spells::registerEvent(Event *event, const pugi::xml_node &) {
    ...
    if (instant) {
        ...
        if (!result.second) {
            std::cout << "[Warning - Spells::registerEvent] Duplicate registered instant spell with words: " << instant
                    ->getWords() << std::endl;
        }
    ...
    if (rune) {
        ...
        if (!result.second) {
            std::cout << "[Warning - Spells::registerEvent] Duplicate registered rune with id: " << rune->
                    getRuneItemId() << std::endl;
        }
        return result.second;
    }
...
bool Spells::loadSpellBoostDefinitionList() {
    spellBoostDefinitions.clear();

    pugi::xml_document doc;
    pugi::xml_parse_result result = doc.load_file("data/XML/spellboosts.xml");
    if (!result) {
        printXMLError("Error - Spells::loadSpellBoostDefinitionList",
                      "data/XML/spellboosts.xml", result);
        return false;
    }

    for (auto spellNode: doc.child("spellboosts").children("spell")) {
        SpellBoostDefinition def;

        pugi::xml_attribute attr;

        if ((attr = spellNode.attribute("id"))) {
            def.id = pugi::cast<uint32_t>(attr.value());
        }

        if ((attr = spellNode.attribute("name"))) {
        ...
        for (auto vocNode: spellNode.child("vocations").children("vocation")) {
            pugi::xml_attribute vocAttr = vocNode.attribute("id");
            if (vocAttr) {
                def.vocations.push_back(
                    pugi::cast<uint16_t>(vocAttr.value())
                );
            }
        }

        for (auto levelNode: spellNode.child("levels").children("level")) {
            SpellBoostLevel lvl;

            if ((attr = levelNode.attribute("index"))) {
                lvl.level = pugi::cast<uint16_t>(attr.value());
            }

            if ((attr = levelNode.attribute("type"))) {
                lvl.type = pugi::cast<uint16_t>(attr.value());
            }

            if ((attr = levelNode.attribute("value"))) {
                lvl.value = pugi::cast<float>(attr.value());
            }

            if ((attr = levelNode.attribute("description"))) {
                lvl.description = attr.as_string();
            }

            def.spellBoostLevels.push_back(lvl);
        }
...
CombatSpell::CombatSpell(Combat *_combat, bool _needTarget, bool _needDirection) : Event(
    &g_spells->getScriptInterface()) {
    combat = _combat;
...
bool InstantSpell::loadFunction(const pugi::xml_attribute &attr) {
    ...
    if (strcasecmp(functionName, "edithouseguest") == 0) {
        ...
        std::cout << "[Warning - InstantSpell::loadFunction] Function \"" << functionName << "\" does not exist." <<
                std::endl;
        return false;
    }
...
bool InstantSpell::Levitate(const InstantSpell *, Creature *creature, const std::string &param) {
    ...
    if (strcasecmp(param.c_str(), "up") == 0) {
        if (currentPos.z != 8) {
            ...
            if (tmpTile == nullptr || (tmpTile->getGround() == nullptr && !tmpTile->hasFlag(
                                           TILESTATE_IMMOVABLEBLOCKSOLID))) {
                tmpTile = g_game.map.getTile(destPos.x, destPos.y, destPos.getZ() - 1);
                if (tmpTile && tmpTile->getGround() && !tmpTile->hasFlag(
                        TILESTATE_IMMOVABLEBLOCKSOLID | TILESTATE_FLOORCHANGE)) {
                    ret = g_game.internalMoveCreature(*player, *tmpTile,
                                                      FLAG_IGNOREBLOCKITEM | FLAG_IGNOREBLOCKCREATURE);
                }
            }
        }
...
ConjureSpell::ConjureSpell(LuaScriptInterface *_interface) : InstantSpell(_interface) {
    aggressive = false;
    conjureId = 0;
    conjureCount = 1;
    reagentId = 0;
...
RuneSpell::RuneSpell(LuaScriptInterface *_interface) : Action(_interface) {
    hasCharges = true;
    runeId = 0;
...
bool RuneSpell::loadFunction(const pugi::xml_attribute &attr) {
    ...
    if (strcasecmp(functionName, "chameleon") == 0) {
        ...
        std::cout << "[Warning - RuneSpell::loadFunction] Function \"" << functionName << "\" does not exist." <<
                std::endl;
        return false;
    }
...
bool RuneSpell::executeUse(Player *player, Item *item, const Position &, Thing *target, const Position &toPosition,
    ...
    postCastSpell(player);
    if (hasCharges && item && g_config.getBoolean(ConfigManager::REMOVE_RUNE_CHARGES) && !player->getTile()->hasFlag(
            TILESTATE_PVPZONE)) {
