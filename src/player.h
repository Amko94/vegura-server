...
public:
    void setSpellBoostLevelByName(const std::string &spellName, uint8_t level);

    uint8_t getSpellBoostLevelByName(const std::string &spellName) const;

    bool loadPlayerSpellBoostLevels(uint32_t playerId);
...
protected:
    ...
    bool dropCorpse(Creature *_lastHitCreature, Creature *mostDamageCreature, bool lastHitUnjustified,
                    bool mostDamageUnjustified) final;
    ...
    std::vector<PlayerSpellBoostLevel> playerSpellBoostLevels;
