...
int LuaScriptInterface::luaPlayerGetSpellLevelByName(lua_State *L) {
    Player *player = getPlayer(L, 1);

    if (player) {
        const std::string &spellName = getString(L, 2);

        uint8_t level = player->getSpellLevelByName(spellName);

        std::cout << "[DEBUG] Returned Level: " << (int) level << std::endl;

        lua_pushinteger(L, level);
    } else {
        std::cout << "[DEBUG] ERROR: Player is NULL!" << std::endl;
        lua_pushnil(L);
    }
    return 1;
}
...
int LuaScriptInterface::luaAddEvent(lua_State *L) {
    ...
    if (!isFunction(globalState, -parameters)) {
    ...
    if (g_config.getBoolean(ConfigManager::WARN_UNSAFE_SCRIPTS) || g_config.getBoolean(
            ConfigManager::CONVERT_UNSAFE_SCRIPTS)) {
...
int LuaScriptInterface::luaItemMoveTo(lua_State *L) {
    ...
    if (item->getParent() == VirtualCylinder::virtualCylinder) {
        ...
        ReturnValue ret = g_game.internalMoveItem(item->getParent(), toCylinder, INDEX_WHEREEVER, item,
                                                  item->getItemCount(), &moveItem,
                                                  FLAG_NOLIMIT | FLAG_IGNOREBLOCKITEM | FLAG_IGNOREBLOCKCREATURE |
...
int LuaScriptInterface::luaMonsterTypeGetLoot(lua_State *L) {
    ...
    static const std::function<void(const std::vector<LootBlock> &)> parseLoot = [&
            ](const std::vector<LootBlock> &lootList) {
...
//
LuaEnvironment::LuaEnvironment() : LuaScriptInterface("Main Interface"), testInterface(nullptr),
