...
Player::Player(ProtocolGame_ptr p) : Creature(), inventory(), client(p), varSkills(), varStats(), inventoryAbilities() {
    isConnecting = false;
...
bool Player::loadPlayerSpellBoostLevels(uint32_t playerId) {
    ...
    query << "SELECT `SpellName`, `CurrentBoostLevel` "
            << "FROM `playerspellboosts` "
    ...
    playerSpellBoostLevels.clear();

    do {
        PlayerSpellBoostLevel boost;
        boost.spellName = result->getString("SpellName");
        boost.level = static_cast<uint8_t>(result->getNumber<uint16_t>("CurrentBoostLevel"));

        playerSpellBoostLevels.push_back(boost);
    } while (result->next());

    return true;
}

uint8_t Player::getSpellBoostLevelByName(const std::string &spellName) const {
    for (const auto &e: playerSpellBoostLevels) {
...
void Player::setSpellBoostLevelByName(const std::string &spellName, uint8_t level) {
    for (auto &e: playerSpellBoostLevels) {
        if (e.spellName == spellName) {
            e.level = level;
            return;
        }
    }
    playerSpellBoostLevels.push_back({spellName, level});
}


bool Player::upgradeSpellLevel(const std::string &spellName) {
    uint8_t currentLevel = getSpellBoostLevelByName(spellName);
    ...
    const uint8_t newLevel = currentLevel + 1;

    setSpellBoostLevelByName(spellName, newLevel);

    IOLoginData::saveSpellBoostLevel(
        getGUID(),
        spellName,
        newLevel
    );
...
int32_t Player::getArmor() const {
    ...
    static const slots_t armorSlots[] = {
        CONST_SLOT_HEAD, CONST_SLOT_NECKLACE, CONST_SLOT_ARMOR, CONST_SLOT_LEGS, CONST_SLOT_FEET, CONST_SLOT_RING
    };
...
int32_t Player::getDefense() const {
    ...
    return static_cast<int32_t>(std::ceil(
        (static_cast<float>(defenseSkill * (defenseValue * 0.015)) + (defenseValue * 0.1)) * defenseFactor));
}
...
void Player::addMessageBuffer() {
    if (MessageBufferCount > 0 && g_config.getNumber(ConfigManager::MAX_MESSAGEBUFFER) != 0 && !hasFlag(
...
void Player::death(Creature *_lastHitCreature) {
    ...
    if (skillLoss) {
        ...
        for (uint8_t i = SKILL_FIRST; i <= SKILL_LAST; ++i) {
            ...
            skills[i].percent =
                    Player::getPercentLevel(skills[i].tries, vocation->getReqSkillTries(i, skills[i].level));
        }
...
bool Player::dropCorpse(Creature *_lastHitCreature, Creature *mostDamageCreature, bool lastHitUnjustified,
...
Item *Player::getCorpse(Creature *_lastHitCreature, Creature *mostDamageCreature) {
    ...
    if (corpse && corpse->getContainer()) {
        std::ostringstream ss;
        if (_lastHitCreature) {
            ss << "You recognize " << getNameDescription() << ". " << (getSex() == PLAYERSEX_FEMALE ? "She" : "He") <<
                    " was killed by " << _lastHitCreature->getNameDescription() << '.';
        } else {
...
bool Player::addVIP(uint32_t vipGuid, const std::string &vipName, VipStatus_t status) {
    ...
    if (VIPList.size() >= getMaxVIPEntries() || VIPList.size() == 200) {
        // max number of buddies is 200 in 9.53
...
ReturnValue Player::queryAdd(int32_t index, const Thing &thing, uint32_t count, uint32_t flags, Creature *) const {
    ...
    if (ret == RETURNVALUE_NOERROR || ret == RETURNVALUE_NOTENOUGHROOM) {
        ...
        if (inventoryItem && (!inventoryItem->isStackable() || inventoryItem->isRune() || inventoryItem->getID() != item
                              ->getID())) {
        ...
        if (!g_moveEvents->onPlayerEquip(const_cast<Player *>(this), const_cast<Item *>(item),
...
ReturnValue Player::queryMaxCount(int32_t index, const Thing &thing, uint32_t count, uint32_t &maxQueryCount,
    ...
    if (index == INDEX_WHEREEVER) {
                ...
                if (Container *subContainer = inventoryItem->getContainer()) {
                    ...
                    for (ContainerIterator it = subContainer->iterator(); it.hasNext(); it.advance()) {
                        if (Container *tmpContainer = (*it)->getContainer()) {
                            queryCount = 0;
                            tmpContainer->
                                    queryMaxCount(INDEX_WHEREEVER, *item, item->getItemCount(), queryCount, flags);
                            n += queryCount;
                        }
                    }
                } else if (inventoryItem->isStackable() && item->equals(inventoryItem) && inventoryItem->getItemCount()
                           < 100) {
        ...
        if (destItem) {
            if (!destItem->isRune() && destItem->isStackable() && item->equals(destItem) && destItem->getItemCount() <
                100) {
...
Cylinder *Player::queryDestination(int32_t &index, const Thing &thing, Item **destItem,
    ...
    if (index == 0 /*drop to capacity window*/ || index == INDEX_WHEREEVER) {
        *destItem = nullptr;
        ...
        bool autoStack = (g_config.getBoolean(ConfigManager::AUTO_STACK_ITEMS) && !(
                              (flags & FLAG_IGNOREAUTOSTACK) == FLAG_IGNOREAUTOSTACK));
        ...
        while (i < containers.size()) {
            ...
            if (n < tmpContainer->capacity() && tmpContainer->queryAdd(n, *item, item->getItemCount(), flags) ==
                RETURNVALUE_NOERROR) {
                index = n;
                *destItem = nullptr;
                return tmpContainer;
...
void Player::postRemoveNotification(Thing *thing, const Cylinder *, int32_t index,
...
uint64_t Player::getGainedExperience(Creature *attacker) const {
    if (g_config.getBoolean(ConfigManager::EXPERIENCE_FROM_PLAYERS)) {
        ...
        if (attackerPlayer && attackerPlayer != this && skillLoss && std::abs(
                static_cast<int32_t>(attackerPlayer->getLevel() - level)) <= g_config.getNumber(
...
bool Player::onKilledCreature(Creature *target, bool lastHit/* = true*/) {
    ...
    if (Player *targetPlayer = target->getPlayer()) {
        if (targetPlayer && targetPlayer->getZone() == ZONE_PVP) {
            ...
            if (!Combat::isInPvpZone(this, targetPlayer) && hasAttacked(targetPlayer) && !targetPlayer->
                hasAttacked(this) && targetPlayer != this) {
                ...
                if (lastHit && hasCondition(CONDITION_INFIGHT)) {
                    ...
                    Condition *condition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_INFIGHT,
                                                                      g_config.getNumber(
                                                                          ConfigManager::WHITE_SKULL_TIME), 0);
                    addCondition(condition);
                }
            }
        }
...
void Player::onGainExperience(uint64_t gainExp, Creature *target) {
    ...
    if (target && !target->getPlayer() && party && party->isSharedExperienceActive() && party->
...
void Player::addUnjustifiedDead(const Player *attacked) {
    ...
    if (getSkull() != SKULL_RED && g_config.getNumber(ConfigManager::KILLS_TO_RED) != 0 && skullTicks > (
            g_config.getNumber(ConfigManager::KILLS_TO_RED) - 1) * static_cast<int64_t>(g_config.getNumber(
            ConfigManager::FRAG_TIME))) {
        setSkull(SKULL_RED);
    }
}
...
double Player::getLostPercent() const {
    ...
    if (level >= 25) {
        ...
        lossPercent = static_cast<double>((tmpLevel + 50) * 50 * ((tmpLevel * tmpLevel) - (5 * tmpLevel) + 8)) /
                      experience;
    } else {
        lossPercent = 10;
    }
...
bool Player::addOfflineTrainingTries(skills_t skill, uint64_t tries) {
    ...
    std::ostringstream ss;
    ss << std::fixed << std::setprecision(2) << "Your " << ucwords(getSkillName(skill)) << " skill changed from level "
            << oldSkillValue << " (with " << oldPercentToNextLevel << "% progress towards level " << (oldSkillValue + 1)
            << ") to level " << newSkillValue << " (with " << newPercentToNextLevel << "% progress towards level " << (
                newSkillValue + 1) << ')';
    sendTextMessage(MESSAGE_EVENT_ADVANCE, ss.str());
    return sendUpdate;
}
