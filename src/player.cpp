...
uint8_t Player::getSpellLevelByName(const std::string &spellName) {
    std::string normalizedName = spellName;
    ...
    auto it = spellLevels.find(normalizedName);
    if (it != spellLevels.end()) {
        return it->second;
    }
    return 0;
}

void Player::loadSpellLevels() {
    Database *db = Database::getInstance();
    std::ostringstream query;

    query << "SELECT `SpellName`, `CurrentBoostLevel` FROM `playerspellboosts` WHERE `PlayerID` = " << guid;
    ...
    if (result) {
        do {
            std::string spellName = result->getString("SpellName");
            uint8_t level = result->getNumber<int>("CurrentBoostLevel");

            std::cout << "[DEBUG] Loaded: " << spellName << " -> " << (int) level << std::endl;
        ...
        std::cout << "[DEBUG] NO RESULT FROM QUERY!" << std::endl;
    }
}
